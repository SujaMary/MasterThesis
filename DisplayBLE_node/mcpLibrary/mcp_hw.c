...
static void mcp_read ( uint8_t reg, uint8_t *buffer);//int mcp_read ( uint8_t reg);

/* SPI write to registers */
static void mcp_write (uint8_t reg, uint8_t *value, uint8_t count);//int mcp_write (uint8_t addr, uint8_t *value, uint8_t count);

/* SPI modifies the registers */
static void mcp_bit_modify (uint8_t reg, uint8_t mask, uint8_t *value);//int mcp_bit_modify (uint8_t addr, uint8_t mask, uint8_t *value);
...
//       Display_printf(hSerial, 0, 0, "SPI Read buff unsuccessful" );
   }
   else {
       /* for debugging*/
...
static void mcp_read_rx (uint8_t rx_reg, uint8_t *buffer)
{
    ...
    spiTransaction.count = 12; //CMD+ID(4)+DLC+data(6)
    ...
    //        Display_printf(hSerial, 0, 0, "mcp_read_rx Received: %d %d %d %d %d %d %d %d %d %d %d %d %d %d", receiveBuffer[0], receiveBuffer[1], receiveBuffer[2], receiveBuffer[3], receiveBuffer[4], receiveBuffer[5], receiveBuffer[6], receiveBuffer[7], receiveBuffer[8], receiveBuffer[9], receiveBuffer[10], receiveBuffer[11], receiveBuffer[12]);
    }
    /* for string */
    /*
    ...
    */
    ...
    /* for uint8 */
    *buffer = receiveBuffer[1];     //CMD
    *(buffer+1) = receiveBuffer[2]; //ID
    *(buffer+2) = receiveBuffer[3];
    *(buffer+3) = receiveBuffer[4];
    *(buffer+4) = receiveBuffer[5];
    *(buffer+5) = receiveBuffer[6]; //dlc
    *(buffer+6) = receiveBuffer[7]; //data
    *(buffer+7) = receiveBuffer[8];
    *(buffer+8) = receiveBuffer[9];
    *(buffer+9) = receiveBuffer[10];
    *(buffer+10) = receiveBuffer[11];
...
static void mcp_load_tx (uint8_t tx_reg, uint8_t *buffer)
{
    ...
    transmitBuffer[1] = *buffer;
    transmitBuffer[2] = *(buffer+1);
    transmitBuffer[3] = *(buffer+2);
    transmitBuffer[4] = *(buffer+3);
    transmitBuffer[5] = *(buffer+4);
    transmitBuffer[6] = *(buffer+5);
    ...
    transmitBuffer[12] = *(buffer+11);
    transmitBuffer[13] = *(buffer+12);
...
void mcp_hw_rx_status (rx_fstatus *status) {
    ...
    if (!transferOK) {
//        Display_printf(hSerial, 0, 0, "SPI read RX status unsuccessful" );
    }
    else {
//        Display_printf(hSerial, 0, 0, "SPI read RX status successful");
//        Display_printf(hSerial, 0, 0, "Transmitted: %d %d %d %d", transmitBuffer[0], transmitBuffer[1], transmitBuffer[2], transmitBuffer[3]);
//        Display_printf(hSerial, 0, 0, "Received: %d %d %d %d\n", receiveBuffer[0], receiveBuffer[1], receiveBuffer[2], receiveBuffer[3]);
    }
...
int mcp_hw_ctl_update (void *value) {
    if( *( ( uint8_t* )value + 1 ) != 0x30 ) // check if address at (*value+1) is TXB0CTRL
    {
        memcpy( ( void* )&reg, value, sizeof( reg_t ) ); //we do '+1' because in can_ctl struct the 1st byte is the things with the bit field, the second byte is the address field
