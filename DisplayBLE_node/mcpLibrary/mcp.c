...
int mcp_msg_load ( TXB_t tx_buff, uint8_t *msg, uint8_t count, uint32_t ID, bool IDE, bool RTR )
{
    ...
    Display_printf (hSerial, 0, 0, "--- Message transmitted ---");
    Display_printf (hSerial, 0, 0, "Tag: %d", *msg);
    ...
    if( ( transfer.id.sidl.ide = IDE ) ) // if extended identifier
    {
        ...
        Display_printf(hSerial, 0, 0, "Transmitted ID: %i", ID);
        //        Display_printf( hSerial, 0, 0, "EID: %i, %i, %i, %i, %i ", transfer.id.sidh, transfer.id.sidl.sidl, transfer.id.sidl.eidt, transfer.id.eidh, transfer.id.eidl  );
      }
    else // if standard identifier
    {
        ...
        Display_printf(hSerial, 0, 0, "Transmitted ID: %i", ID);
        //           Display_printf( hSerial, 0, 0, "SID: %i, %i, %i, %i, %i ", transfer.id.sidh, transfer.id.sidl.sidl, transfer.id.sidl.eidt, transfer.id.eidh, transfer.id.eidl  );
    }
    ...
    //    Display_printf (hSerial, 0, 0, "--- Message transmitted ---");
    //    Display_printf (hSerial, 0, 0, "\nData tag: %d", *transfer.payload.buff);
...
//int mcp_msg_read ( RXB_t rx_buffer, uint16_t *msg, uint8_t *count, uint32_t *ID, bool *IDE, bool *RTR ) //for uin16 -- last working
//int mcp_msg_read ( RXB_t rx_buffer, uint8_t *msg, uint8_t *count, uint32_t *ID, bool *IDE, bool *RTR )//changed uint8_t *msg to char *msg
int mcp_msg_read ( RXB_t rx_buffer, uint16_t *msg, uint8_t *count, uint32_t *ID, bool *IDE, bool *RTR ) //for data + tag
{
    ...
    *count = transfer.payload.dlc.len; // copy count
    ...
    /* for debugging */
    //    Display_printf(hSerial, 0, 0, "count %d", *count);
    //    Display_printf(hSerial, 0, 0, "Data in payload: %d %d %d %d %d %d", transfer.payload.buff[0], transfer.payload.buff[1], transfer.payload.buff[2], transfer.payload.buff[3], transfer.payload.buff[4], transfer.payload.buff[5]);
    //    Display_printf(hSerial, 0, 0, "Payload data %d %d %d", *count, *transfer.payload.buff, *(transfer.payload.buff+1));

    memcpy( ( void* )msg, ( void* )transfer.payload.buff, *count); // copy received message

    /* for debugging*/

    //    Display_printf (hSerial, 0, 0, "Data in msg: %d %d %d", *msg, *(msg+1), *(msg+2));
    //    Display_printf (displayHandle, 0, 0, "Payload: %s", transfer.payload.buff);
    //    Display_printf(hSerial, 0, 0, "count: %d, data: %s", *count, msg);
    //    Display_printf(hSerial, 0, 0, "msg %d %d", *msg);
