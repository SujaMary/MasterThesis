...
// App event passed from stack modules
typedef struct
{
...
// Container to store passcode data when passing from gapbondmgr callback to app event.
typedef struct
{
  uint8_t state;
  uint16_t connHandle;
...
// Container to store advertising event data when passing from advertising callback to app event
typedef struct
{
  uint32_t event;
  void *pBuf;
...
// Container to store information from clock expiration using a flexible array since data is not always needed
typedef struct
{
  uint8_t event;
  uint8_t data[];
...
uint16_t combined[3];
static uint16_t glove_data_finger1 = 0, glove_data_finger2 = 0, glove_tag= 0, keypad_data= 0, keypad_tag = 0;

uint8_t data_tag;
static uint16_t tag, finger1, finger2, key;
char spacer[1] = "#";
char *str;
char strtag[10], strkey[10], strfinger1[10];
uint32_t numconcat;
...
// Clock instance for internal periodic events. Only one is needed since GattServApp will handle notifying all connected GATT clients
...
static void SimplePeripheral_init(void)
{
  ...
  if (spiHandle == NULL) {
      Display_printf(hSerial, 0, 0, "SPI open unsuccessful");
      while (1);
  }
  ...
  Display_printf(hSerial, 0, 0, "Node 2\n");
  Display_printf(hLcd, 5, 3, "Node 2");
  sleep(2);
  Display_clear(hLcd);
  ...
  if( mcp_init(OPMODE_NORMAL))
      ...
      Display_printf(hSerial, 0, 0, "MCP25625 initialized");

  /* Configure receive buffer filters */
  if( mcp_filter_config( RXF_0, 0x7E8, 0x7E8, true ) || \
      mcp_filter_config( RXF_1, 0x7E8, 0x7E8, true ) || \
      mcp_filter_config( RXF_2, 0x7EA, 0x7EA, true ) || \
      mcp_filter_config( RXF_3, 0x7EA, 0x7EA, true ) || \
      mcp_filter_config( RXF_4, 0x7EA, 0x7EA, true ) || \
      mcp_filter_config( RXF_5, 0x7EA, 0x7EA, true ) )

      Display_printf(hSerial, 0, 0," - Filter Config Error\r\n" );
   else
      Display_printf(hSerial, 0, 0, " - Filter Config Done\r\n" );

  /* Configure receive buffer masks */
  if( mcp_mask_config( RXB0, 0x07FF, 0x1FFFFFFF ) || \
      mcp_mask_config( RXB1, 0x07FF, 0x1FFFFFFF ) )
      Display_printf(hSerial, 0, 0, " - Mask Config Error\r\n" );
  else
      Display_printf(hSerial, 0, 0," - Mask Config Done\r\n" );

  /* Configure receive buffers */
  if( mcp_rx_config( RXB0, RX_MODE_EID, true, false ) || \
      mcp_rx_config( RXB1, RX_MODE_EID, false, false ) )
      Display_printf(hSerial, 0, 0," - RX Configuration Error\r\n" );
  else
      Display_printf(hSerial, 0, 0," - RX Config Done\r\n" );
...
static void SimplePeripheral_taskFxn(UArg a0, UArg a1)
{
  ...
  {
    /* Generate data tag */
    for (data_tag = 1; data_tag <= 50; data_tag++) {
    /* Call CAN function */
    canBusFxn();
        ...
        case HCI_COMMAND_STATUS_EVENT_CODE:
        {
          ...
          switch ( pMyMsg->cmdOpcode )
          {
            case HCI_LE_SET_PHY:
            {
              ...
              {
//                  Display_printf(hSerial, SP_ROW_STATUS_1, 0,
//                          "PHY Change failure, peer does not support this");
//                      Display_printf(hSerial, 0, 0, "PHY Change failure, peer does not support this");
              }
              else
              {
//                Display_printf(hSerial, SP_ROW_STATUS_1, 0,
...
//              Display_printf(hSerial, SP_ROW_STATUS_2, 0,
//                             "PHY Updated to %s",
...
static uint8_t SimplePeripheral_processGATTMsg(gattMsgEvent_t *pMsg)
{
  if (pMsg->method == ATT_FLOW_CTRL_VIOLATED_EVENT)
  {
    ...
    // Display_printf(hSerial, SP_ROW_STATUS_1, 0, "FC Violated: %d", pMsg->msg.flowCtrlEvt.opcode);
    // Display_printf(hSerial, 0, 0, "FC Violated: %d", pMsg->msg.flowCtrlEvt.opcode);
  }
  else if (pMsg->method == ATT_MTU_UPDATED_EVENT)
  {
    // MTU size updated
    // Display_printf(hSerial, SP_ROW_STATUS_1, 0, "MTU Size: %d", pMsg->msg.mtuEvt.MTU);
    // Display_printf(hSerial, 0, 0, "MTU Size: %d", pMsg->msg.mtuEvt.MTU);
  }

  // Free message payload. Needed only for ATT Protocol messages
  GATT_bm_free(&pMsg->msg, pMsg->method);

  // Safe to free the incoming message
  return (TRUE);
}
...
static void SimplePeripheral_processAppMsg(spEvt_t *pMsg)
{
//    Display_printf(hSerial, 0, 0, "in process App msg");
  bool dealloc = TRUE;
...
//      SimplePeripheral_handleKeys(*(uint8_t*)(pMsg->pData));
    ...
    case SP_TAG_MATCH_EVT:
    {
      SimplePeripheral_processTagMatchEvt();
      break;
    }
    default:
      break;
...
//        Display_printf(hSerial, SP_ROW_STATUS_1, 0, "Initialized");
//        Display_printf(hSerial, 1, 0, "Initialized");
...
//        tbm_setItemStatus(&spMenuMain, SP_ITEM_SELECT_CONN,SP_ITEM_AUTOCONNECT);
    ...
    case GAP_LINK_TERMINATED_EVENT:
    {
      ...
      // Display_clearLine(hSerial, SP_ROW_CONNECTION);
      break;
    }
    ...
    case GAP_LINK_PARAM_UPDATE_EVENT:
    {
      ...
      if(pPkt->status == SUCCESS)
      {
        ...
        // Display_printf(hSerial, SP_ROW_STATUS_2, 0, "Link Param Updated: %s",
    ...
    default:
       //  Display_clearLines(hSerial, SP_ROW_STATUS_1, SP_ROW_STATUS_2);
      break;
  }
}
 ...
 * @brief   Callback from Simple Profile indicating a characteristic value change.
 ...
 * @brief   Process a pending Simple Profile characteristic value change event.
 * @param   paramID - parameter ID of the value that was changed.
 */
static void SimplePeripheral_processCharValueChangeEvt(uint8_t paramId)
{
  ...
  switch(paramId)
  {
    case SIMPLEPROFILE_CHAR1:
      ...
      Display_printf(hSerial, 0, 0, "Char 1: %d", (uint16_t)newValue);
      break;

    case SIMPLEPROFILE_CHAR3:
      SimpleProfile_GetParameter(SIMPLEPROFILE_CHAR3, &newValue);
      Display_printf(hSerial, 0, 0, "Char 3: %d", (uint16_t)newValue);
      break;
 ...
 * @brief   Perform a periodic application task. This function gets called every five seconds (SP_PERIODIC_EVT_PERIOD).
 * @param   None.
 * @return  None.
 */
static void SimplePeripheral_performPeriodicTask(void)
{
  ...
  {
    // Call to set the value of the fourth characteristic in the profile.
    //    SimpleProfile_SetParameter(SIMPLEPROFILE_CHAR4, sizeof(uint8_t),
    //                               &valueToCopy);
  }
}
 ...
 * @brief   Read the current RPA from the stack and update display if the RPA has changed.
 * @param   None.
 * @return  None.
 */
static void SimplePeripheral_updateRPA(void)
...
//    Display_printf(hSerial, SP_ROW_RPA, 0, "RP Addr: %s",
...
bool SimplePeripheral_doSetConnPhy(uint8 index)
{
  ...
  {
  //    Display_printf(hSerial, SP_ROW_STATUS_1, 0, "Connection handle is not in the connList !!!");
    return FALSE;
  }
  ...
  // Display_printf(hSerial, SP_ROW_STATUS_1, 0, "PHY preference: %s",
...
//      Display_printf(hSerial, SP_ROW_ADVSTATE, 0, "Adv Set %d Enabled",
//                     *(uint8_t *)(pEventData->pBuf));
      break;

    case GAP_EVT_ADV_END_AFTER_DISABLE:
//      Display_printf(hSerial, SP_ROW_ADVSTATE, 0, "Adv Set %d Disabled",
//                     *(uint8_t *)(pEventData->pBuf));
      break;
...
//      Display_printf(hSerial, SP_ROW_ADVSTATE, 0, "Adv Set %d disabled after conn %d",
...
//      Display_printf(hSerial, SP_ROW_CONNECTION, 0, "Pairing started");
      break;

    case GAPBOND_PAIRING_STATE_COMPLETE:
      if (status == SUCCESS)
      {
//        Display_printf(hSerial, SP_ROW_CONNECTION, 0, "Pairing success");
      }
      else
      {
//        Display_printf(hSerial, SP_ROW_CONNECTION, 0, "Pairing fail: %d", status);
      }
      break;
    ...
    case GAPBOND_PAIRING_STATE_ENCRYPTED:
      if (status == SUCCESS)
      {
//        Display_printf(hSerial, SP_ROW_CONNECTION, 0, "Encryption success");
      }
      else
      {
//        Display_printf(hSerial, SP_ROW_CONNECTION, 0, "Encryption failed: %d", status);
      }
      break;
    ...
    case GAPBOND_PAIRING_STATE_BOND_SAVED:
      if (status == SUCCESS)
      {
//        Display_printf(hSerial, SP_ROW_CONNECTION, 0, "Bond save success");
      }
      else
      {
//        Display_printf(hSerial, SP_ROW_CONNECTION, 0, "Bond save failed: %d", status);
      }
      break;
...
static void SimplePeripheral_processPasscode(spPasscodeData_t *pPasscodeData)
{
  ...
  {
//    Display_printf(hSerial, SP_ROW_CONNECTION, 0, "Passcode: %d",
...
//	  Display_printf(hSerial, SP_ROW_AC, 0, "AutoConnect enabled: Group A");
    }
    else if (index == 2)
    {
...
//      Display_printf(hSerial, SP_ROW_AC, 0, "AutoConnect enabled: Group B");
    }
    else
    {
      if (autoConnect)
...
//      Display_printf(hSerial, SP_ROW_AC, 0, "AutoConnect disabled");
    }
//    tbm_goTo(&spMenuMain);
...
//        Display_printf(hSerial, SP_ROW_RSSI, 0,
...
//                       connList[index].rssiAvg);
    ...
    case HCI_LE_READ_PHY:
    {
      if (status == SUCCESS)
      {
//        Display_printf(hSerial, SP_ROW_RSSI + 2, 0, "RXPh: %d, TXPh: %d",
...
/*
 * @fn      canBusFxn
 * @brief   transmit and receive CAN bus message, compare received messages
 * @param   none
 * @return  none
 *
 * # SOLUTION #
...
void canBusFxn(void) {
    ...
    /* Transmit message */
    if( !mcp_msg_load(TXB0, &data_tag, 1, EID, true, false) && \
            !mcp_msg_send(TXB0))
    {
        Display_printf(hSerial, 0, 0, "MESSAGE SENT\n");
    }
    else
    {
        Display_printf(hSerial, 0, 0, "ERROR\n");
    }

    /* Check RXB0 for message - set to receive flex sensor node data */
    if (mcp_msg_ready (RXB0))
    {
        /* Read message from RXB0 */
        if(mcp_msg_read(RXB0, combined, &count, &ID, &RX_IDE, &RX_RTR))
        {
            Display_printf(hSerial, 0, 0, "RX Error");
        }

        /* Separate received message to data and tag */
        glove_data_finger1 = combined[0];
        glove_data_finger2 = combined[1];
        glove_tag  = combined[2];

        Display_printf(hSerial, 0, 0, "--- Message received ---");
        Display_printf(hSerial, 0, 0, "Receive buffer: RX0");
        Display_printf(hSerial, 0, 0, "Received ID: %i", ID);
        Display_printf(hSerial, 0, 0, "Data - Finger 1: %d Finger 2: %d \r\nTag: %d\n", combined[0], combined[1], combined[2]);

        Display_printf(hLcd, 0, 0, "Receive buffer: RX0");
        Display_printf(hLcd, 2, 0, "Received ID: %i", ID);
        Display_printf(hLcd, 3, 0, "Data - Finger 1: %d", combined[0]);
        Display_printf(hLcd, 4, 0, "Data - Finger 2: %d", combined[1]);
        Display_printf(hLcd, 5, 0, "Tag: %d", combined[2]);

        sleep(1);
        Display_clear(hLcd);
    }

    /* Compare tags from received messages */
    if (glove_tag == keypad_tag)
    {
        /* Call functions for tag match */
        SimplePeripheral_TagMatchCB();

        tag = keypad_tag;

        Display_printf(hSerial, 0, 0, "Tag %d matched!", tag);
        Display_printf(hSerial, 0, 0, "Data - Finger 1: %d Finger 2: %d Key: %c\n", glove_data_finger1, glove_data_finger2, keypad_data);

        Display_printf(hLcd, 11, 0, "Tag %d matched!", tag);
        Display_printf(hLcd, 13, 0, "Finger 1: %d", glove_data_finger1);
        Display_printf(hLcd, 14, 0, "Finger 2: %d", glove_data_finger2);
        Display_printf(hLcd, 15, 0, "Key: %c", keypad_data);
    }

    /* Check RXB1 for message - set to receive keypad node data*/
    if (mcp_msg_ready (RXB1))
    {
        /* Read message from RXB1 */
        if(mcp_msg_read(RXB1, combined, &count, &ID, &RX_IDE, &RX_RTR))
        {
            Display_printf(hSerial, 0, 0, "RX Error");
        }

        /* Separate received message to data and tag */
        keypad_data = combined[0];
        keypad_tag  = combined[1];

        Display_printf(hSerial, 0, 0, "--- Message received ---");
        Display_printf(hSerial, 0, 0, "Receive buffer: RX1");
        Display_printf(hSerial, 0, 0, "Received ID: %i", ID);
        Display_printf(hSerial, 0, 0, "Data - Key: %c Tag = %d\n", combined[0], combined[1]);

        Display_printf(hLcd, 0, 0, "Receive buffer: RX1");
        Display_printf(hLcd, 2, 0, "Received ID: %i", ID);
        Display_printf(hLcd, 3, 0, "Data - Key: %c", combined[0]);
        Display_printf(hLcd, 4, 0, "Tag = %d", combined[1]);
    ...
    if (glove_tag == keypad_tag)
    {
        ...
        Display_printf(hLcd, 10, 0, "Tag %d matched!", tag);
        Display_printf(hLcd, 11, 0, "Tag %d matched!", tag);
        Display_printf(hLcd, 13, 0, "Finger 1: %d", glove_data_finger1);
        Display_printf(hLcd, 14, 0, "Finger 2: %d", glove_data_finger2);
        Display_printf(hLcd, 15, 0, "Key: %c", keypad_data);
...
/*
 * @fn      SimplePeripheral_TagMatchCB
 * @brief   enqueues message to stack on event
 * @param   none
 * @return  none
 *
 * # SOLUTION #
 */
static void SimplePeripheral_TagMatchCB(){
    SimplePeripheral_enqueueMsg(SP_TAG_MATCH_EVT, NULL);
}

/*
 * @fn      SimplePeripheral_processTagMatchEvt
 * @brief   process when tags match
 * @param   none
 * @return  none
 *
 * # SOLUTION #
 */
static void SimplePeripheral_processTagMatchEvt(void){
    uint32_t tagToCopy;
    tagToCopy = tag;

    /* Set matched tag to characteristic 4 value */
    SimpleProfile_SetParameter(SIMPLEPROFILE_CHAR4, sizeof(tagToCopy), &tagToCopy);

    /* Toggle LED to indicate match */
