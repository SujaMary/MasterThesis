...
int mcp_msg_load ( TXB_t tx_buff, uint16_t *msg, uint8_t count, uint32_t ID, bool IDE, bool RTR )
{
    ...
    if( ( transfer.id.sidl.ide = IDE ) ) // if extended identifier
        ...
        //        Display_printf( displayHandle, 0, 0, "EID: %i, %i, %i, %i, %i ", transfer.id.sidh, transfer.id.sidl.sidl, transfer.id.sidl.eidt, transfer.id.eidh, transfer.id.eidl  );//uncomment
      }
        ...
        //        Display_printf( displayHandle, 0, 0, "SID: %i, %i, %i, %i, %i ", transfer.id.sidh, transfer.id.sidl.sidl, transfer.id.sidl.eidt, transfer.id.eidh, transfer.id.eidl  );//uncomment
    }
    ...
    /* for debugging */
    //    Display_printf (displayHandle, 0, 0, "Data in msg: %d %d %d", *msg, *(msg+1), *(msg+2));
    //    Display_printf (displayHandle, 0, 0, "Data into msg_load from main: %s", msg);

    memcpy( ( void* )&transfer.payload.buff, ( void* )msg, count); // copy message

    /* for debugging */
    //    Display_printf(displayHandle, 0, 0, "Data in payload: %d %d %d %d %d %d", transfer.payload.buff[0], transfer.payload.buff[1], transfer.payload.buff[2], transfer.payload.buff[3], transfer.payload.buff[4], transfer.payload.buff[5]);
    //    Display_printf (displayHandle, 0, 0, "Data copied into payload in msg_load: %d %s %s", count, *transfer.payload.buff, transfer.payload.buff);
    //    Display_printf (displayHandle, 0, 0, "Data copied into payload in msg_load: %d %d %d", count, *transfer.payload.buff, *(transfer.payload.buff+1));
...
int mcp_msg_read ( RXB_t rx_buffer, uint8_t *msg, uint8_t *count, uint32_t *ID, bool *IDE, bool *RTR )
{
    ...
    *count = transfer.payload.dlc.len; // copy count
    ...
    memcpy( ( void* )msg, ( void* )transfer.payload.buff, *count );
