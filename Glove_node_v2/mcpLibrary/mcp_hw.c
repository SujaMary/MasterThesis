...
static void mcp_read ( uint8_t reg, uint8_t *buffer);//add static back

/* SPI write to registers */
static void mcp_write (uint8_t reg, uint8_t *value, uint8_t count);//add static back

/* SPI modifies the registers */
static void mcp_bit_modify (uint8_t reg, uint8_t mask, uint8_t *value);//add static back
...
void mcp_read_buff ( uint8_t reg) {
   ...
   if (!transferOK) {
      ...
      Display_printf(displayHandle, 0, 0, "Received: %d %d %d %d %d %d %d %d %d %d %d %d %d\n", receiveBuffer[0], receiveBuffer[1], receiveBuffer[2], receiveBuffer[3], receiveBuffer[4], receiveBuffer[5], receiveBuffer[6], receiveBuffer[7], receiveBuffer[8], receiveBuffer[9], receiveBuffer[10], receiveBuffer[11], receiveBuffer[12], receiveBuffer[13]);
   }
...
static void mcp_read_rx (uint8_t rx_reg, uint8_t *buffer)
{
    ...
    spiTransaction.count = 10; //CMD+ID(4)+DLC+data(4)
    ...
    //        Display_printf(displayHandle, 0, 0, "Received: %d %d %d %d %d %d %d %d %d %d \n", receiveBuffer[0], receiveBuffer[1], receiveBuffer[2], receiveBuffer[3], receiveBuffer[4], receiveBuffer[5], receiveBuffer[6], receiveBuffer[7], receiveBuffer[8], receiveBuffer[9]);
    }
...
static void mcp_load_tx (uint8_t tx_reg, uint8_t *buffer)
{
    ...
    transmitBuffer[6] = *(buffer+5);    //data 0 N F
    transmitBuffer[7] = *(buffer+6);    //data 1 o i
    transmitBuffer[8] = *(buffer+7);    //data 2   n
    transmitBuffer[9] = *(buffer+8);    //data 3 d g
    transmitBuffer[10] = *(buffer+9);   //data 4 a e
    transmitBuffer[11] = *(buffer+10);  //data 5 t r
    transmitBuffer[12] = *(buffer+11);  //data 6 a 1
    transmitBuffer[13] = *(buffer+12);  //data 7   2
    ...
    */
    ...
    transmitBuffer[4] = *(buffer+3);    //eidl
    transmitBuffer[5] = *(buffer+4);    //dlc
    transmitBuffer[6] = *(buffer+5);    //data

    spiTransaction.count = 7; //CMD+ID(4)+DLC+data(8)
     */
    ...
    transmitBuffer[5] = *(buffer+4);    //dlc
    transmitBuffer[6] = *(buffer+5);    //data
    transmitBuffer[7] = *(buffer+6);
    transmitBuffer[8] = *(buffer+7);
    transmitBuffer[9] = *(buffer+8);
    transmitBuffer[10] = *(buffer+9);
    transmitBuffer[11] = *(buffer+10);

    spiTransaction.count = 12; //CMD+ID(4)+DLC+data(6)--data *2+tag
    ...
    //        Display_printf(displayHandle, 0, 0, "Data copied into registers in mcp_load_tx Transmitted: %d %d %d %d %d %d %d %d %d %d %d %d %d %d", transmitBuffer[0], transmitBuffer[1], transmitBuffer[2], transmitBuffer[3], transmitBuffer[4], transmitBuffer[5], transmitBuffer[6], transmitBuffer[7], transmitBuffer[8], transmitBuffer[9], transmitBuffer[10], transmitBuffer[11], transmitBuffer[12], transmitBuffer[13]);
